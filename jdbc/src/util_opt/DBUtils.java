...
//        if (size != holderValues.size()) throw new IllegalArgumentException("占位符和值的数量不同");
    ...
    // 处理事务的版本
    public static void execute(Connection connection, String sql, Object... values) {
    ...
    public static void execute() {
        execute(sql, holderValues.toArray());
    }
    ...
    public static <T> Optional<List<T>> query(Class<T> aClass) {
        return query(aClass, sql, holderValues.toArray());
    }
     ...
     * @return 表对应的Java类的一个对象
     */
    public static <T> Optional<List<T>> query(Class<T> aClass, String sql, Object... holderValues) {
        try (var cxn = getConnection()) {
    ...
    public static <T> Optional<List<T>> query(Connection connection, Class<T> aClass, String sql,
        ...
        try (var stmt = connection.prepareStatement(sql)) {
            for (int i = 0; i < holderValues.length; i++) {
                stmt.setObject(i + 1, holderValues[i]);
            }
